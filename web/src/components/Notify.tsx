    ...
    <Menu className="w-72">
      ...
      {data && data.length > 0 ? (
        data.map((item: any, index: any) => (
          <Menu.Item className="border-t border-b">
            <Row
              key={index}
              justify="space-between"
              className={index < readIndex ? 'border-l-4 border-blue-500' : ''}
            >
              ...
              <Col xs={20}>
                <Space direction="vertical" size={2} className="ml-2">
                  <div dangerouslySetInnerHTML={getContent(item[notificationMsg])} />
                  {showDate && (
  ...
  return (
    <>
      <Dropdown
        ...
        arrow
      >
        <div className="cursor-pointer" onClick={markAsRead}>
